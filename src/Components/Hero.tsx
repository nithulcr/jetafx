...
const Hero = () => {
  ...
  const heroSecondGridRef = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    const timeout = setTimeout(() => {
      if (!sectionRef.current || !heroSecondGridRef.current) return;
      const section = sectionRef.current;
      const heroSecondGrid = heroSecondGridRef.current;
      ...
      const heroSecondGridCtx = gsap.context(() => {
        gsap.fromTo(
          ".blurry-card",
          { opacity: 0, x: 60, y: 20 },
          {
  ...
  return (
    <div className=' hero overflow-hidden'>
      <section ref={sectionRef} className="relative lg:pb-2 lg:pt-20 lg:pb-6">
